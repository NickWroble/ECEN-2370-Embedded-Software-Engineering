...
/***************************************************************************//**
 * @brief Opens the BLE connection using the LEUART bus protocol
 *
 * @details Initialized values for the LEUART_OPEN_STRUCT to be used for LEUART comms,
 * Calls leuart_open() to enable LEUART comms
 *
 *@param[in] uint32_t tx_event
 *@param[in] uint32_t rx_event
 *
 ******************************************************************************/
...
void ble_open(uint32_t tx_event, uint32_t rx_event){
	...
	leuart_open_struct.rx_done_evt = rx_event;
	leuart_open_struct.rx_en = HM10_RXENABLE;
	leuart_open_struct.rx_loc = LEUART0_RX_ROUTE;
	leuart_open_struct.rx_pin_en = LEUART_ROUTEPEN_RXPEN;

	leuart_open_struct.tx_done_evt = tx_event;
	leuart_open_struct.tx_en = HM10_TXENABLE;
	leuart_open_struct.tx_loc = LEUART0_TX_ROUTE;
	leuart_open_struct.tx_pin_en = LEUART_ROUTEPEN_TXPEN;

	leuart_open(HM10_LEUART0, &leuart_open_struct);
}


/***************************************************************************//**
 * @brief Writes data onto the BLE LEUART bus
 *
 * @details calls leuart_start()
 *
 ******************************************************************************/
void ble_write(char* string, uint32_t callback){
	leuart_start(HM10_LEUART0, string, callback);
}
...
bool ble_test(char *mod_name){
	...
	// ANSWER:
	// Polling is a delay based data collection method that requires the
	// CPU to periodically gather data, so the CPU is always on. Interrupts
	// can wake a CPU up from sleep, which saves energy when compared to polling
	//
	// How does interrupts benefit the system for low energy operation?
	// ANSWER:
	// Interrupts allow the CPU to wake from sleep and immediately perform  an operation,
	// polling requires the CPU to always be on

	// How does interrupts benefit the system that has multiple tasks?
	// ANSWER:
	// When an interrupt occurs, the data from the current process is first saved,
	// and then the interrupt is handled by the interrupt service routine. After the
	// the ISR, the data from the interrupted process is restored and the computer
	// returns to the interrupted process. Because of this, and the fact that an interrupt
	// occurs depends on external I/O, allowing for computation on demand
...
//	leuart_cmd_write(HM10_LEUART0, (LEUART_CMD_CLEARRX | LEUART_CMD_CLEARTX));
	...
	// DSD HM10 module?
	// ANSWER:
	// The name cannot change during an active connection
	...
	// ANSWER:
	// It will return "OK+<test>", test is the string that was sent
