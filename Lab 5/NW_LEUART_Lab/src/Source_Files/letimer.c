...
/***************************************************************************//**
 * @brief Driver to open an set an LETIMER peripheral in PWM mode
 *
 * @details
 ...
 * 	 functions need to be serviced. This function also sets up routines for the Interrupt
 * 	 Service Handle. Lastly the function disables sleep mode when letimer is running
 *
 * @note This function is normally called once to initialize the peripheral and the
 *   function letimer_start() is called to turn-on or turn-off the LETIMER PWM
 *   operation. This function also enable nested interrupts
 *
 * @param[in] letimer
...
/***************************************************************************//**
 * @brief Function that handles interrupts
 *
 *
 * @details  Function first disables interrupts that are not required for this application.
 * Next the interrupt flag register is cleared to enable multiple calls to the ISR. Each required
 * interrupt is then serviced and scheduled.
 *
 *
 * @note Each required interrupt has an assertion in order to ensure each interrupt is serviced
 * properly
 *
 *
 ******************************************************************************/
...
/***************************************************************************//**
 * @brief Enables the letimer to start
 *
 *
 * @details This function blocks and unblocks sleep mode depending on if the letimer
 * is running with the use of a while loop
 *
 *
 * @note Sleep is blocked if letimer is enabled and sleep is unblocked if letimer is
 * disabled.
 *
 *
 * @param[in] letimer
 * variable representing letimer
 *
 *
 *  @param[in] enable
 *  Used in order to compare the letimer status register
 *
 ******************************************************************************/
